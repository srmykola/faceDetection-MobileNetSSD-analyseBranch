...
def depthwiseConvBlockClassification(inputs, pointwise_conv_filters, alpha,
                          depthMultiplier=1, strides=(1, 1), block_id=1):
...
def depthwiseConvBlockAnalyseBackbone(inputs, pointwise_conv_filters, alpha,
                          depthMultiplier = 1, strides = (1, 1), block_id = 1,
                          layer_name = 'analyse'):
    ...
    x = DepthwiseConv2D( (3, 3),
                        ...
                        strides = strides,
                        use_bias = False,
                        name = layer_name + 'conv_dw_%d' % block_id )( inputs )
    x = BatchNormalization( axis = channel_axis, name = layer_name + 'conv_dw_%d_bn' % block_id )( x )
    x = Activation( relu6, name = layer_name + 'conv_dw_%d_relu' % block_id )( x )

    x = Conv2D( pointwise_conv_filters, (1, 1),
               padding = 'same',
               use_bias = False,
               strides = (1, 1),
               name = layer_name + 'conv_pw_%d' % block_id )( x )
    x = BatchNormalization( axis = channel_axis, name = layer_name + 'conv_pw_%d_bn' % block_id )( x )
    return Activation( relu6, name = layer_name + 'conv_pw_%d_relu' % block_id )( x )

def depthwiseConvBlockDetection(input, layer_name, strides = (1,1),
                          kernel_size = 3,
                          pointwise_conv_filters=32, alpha=1.0, depthMultiplier=1,
                          padding = 'valid',
                          data_format = None,
...
def depthwiseConvBlockAnalyseNeck(input, layer_name, strides = (1,1),
                                  kernel_size = 3,
                                  pointwise_conv_filters=32, alpha=1.0, depthMultiplier = 1,
                                  padding = 'valid',
                                  data_format = None,
